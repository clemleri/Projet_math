...
def evaluer_cnf(formule,list_var):
    ...
    for i in range(len(formule)):
        lst_result.append(evaluer_clause(copy_form[i], list_var))


    if False in lst_result and None not in lst_result:
        return False
    elif None in lst_result:
        return None 
    else:
        return True
...
def retablir_for(formule_init,list_chgmts):
    '''Arguments : une formule initiale et une liste de changements à apporter sur un ensemble 
    de variables (chaque changement étant une liste [i,bool] avec i l'index qu'occupe la variable
...
def progress_simpl_for_dpll(formule,list_var,list_chgmts,list_sans_retour):
    ...
    for i in range(1,len(list_var)):
        if any(i in clauses for clauses in formule) and not any(-(i) in clauses for clauses in formule):
            list_var[i-1] = True
            list_chgmts.append([i-1, list_var[i-1]])
            list_sans_retour.append(i-1)
            return init_formule_simpl_for(formule, list_var), list_var, list_chgmts, list_sans_retour
        elif any(-(i) in clauses for clauses in formule) and not any(i in clauses for clauses in formule):
            list_var[i-1] = False
            list_chgmts.append([i-1, list_var[i-1]])
            list_sans_retour.append(i-1)
            return init_formule_simpl_for(formule, list_var), list_var, list_chgmts, list_sans_retour
    ...
    for clause in formule:
        if len(clause) == 1:
            if clause[0] in list_sans_retour:
                ...
                return init_formule_simpl_for(formule, list_var), list_var, list_chgmts, list_sans_retour
    formule, _, _ = progress_simpl_for(formule, list_var, list_chgmts)
...
def retour(list_var,list_chgmts):
    ...
    if list_chgmts == []:
        ...
        for i in range(len(list_chgmts)-1,-1,-1):
            if list_var[list_chgmts[i][0]] == False:
                list_var[list_chgmts[i][0]] = None
                list_chgmts.pop(i)
            elif list_var[list_chgmts[i][0]] == True:
                list_var[list_chgmts[i][0]] = False
                list_chgmts[i][1] = False
                return list_var, list_chgmts
    return list_var, list_chgmts
...
def for_conj_sudoku(n):
    ...
    '''
    variables = [[f"p{i}_{j}_{k}" for k in range(1, n**2 + 1)] for j in range(n**2) for i in range(n**2)]
    flat_variables = [item for sublist in variables for item in sublist]

    clauses_c1 = []
    for i in range(n**2):
        for k in range(1, n**2 + 1):
            clause = [flat_variables[j + i * n**2].format(k) for j in range(n**2)]
            clauses_c1.append(clause)

    clauses_c2 = []
    for j in range(n**2):
        for k in range(1, n**2 + 1):
            clause = [flat_variables[i * n**2 + j].format(k) for i in range(n**2)]
            clauses_c2.append(clause)

    clauses_c3 = []
    for x in range(n):
        for y in range(n):
            for k in range(1, n**2 + 1):
                clause = [flat_variables[(i + x * n) * n**2 + (j + y * n)].format(k) for i in range(n) for j in range(n)]
                clauses_c3.append(clause)

    clauses_c4 = [[flat_variables[i * n**2 + j].format(k) for k in range(1, n**2 + 1)] for i in range(n**2) for j in range(n**2)]

    clauses_c5 = []
    for i in range(n**2):
        for j in range(n**2):
            for k1 in range(1, n**2):
                for k2 in range(k1 + 1, n**2 + 1):
                    clauses_c5.append(["-" + flat_variables[i * n**2 + j].format(k1), "-" + flat_variables[i * n**2 + j].format(k2)])

    cnf_formula = clauses_c1 + clauses_c2 + clauses_c3 + clauses_c4 + clauses_c5
