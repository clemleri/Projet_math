...
NOM = "Lerouley" # A REMPLIR
PRENOM = "Clément" # A REMPLIR
...
def evaluer_clause(clause,list_var):
    ...
    for variable in clause:
        if list_var[abs(variable)-1] == None:
            lst_bool_result.append(None)
            continue
        if list_var[abs(variable)-1] == False and variable < 0:
            bool_result = True
        elif list_var[abs(variable)-1] == True and variable > 0:
            bool_result = True
        elif (list_var[abs(variable)-1] == True and variable < 0) or (list_var[abs(variable)-1] == False and variable > 0):
            bool_result = False
        if bool_result:
            return True
        lst_bool_result.append(bool_result)
    if None in lst_bool_result:
        return None
    elif None not in lst_bool_result and False in lst_bool_result:
        return False
...
def evaluer_cnf(formule,list_var):
    ...
    for clause in formule:
        lst_result.append(evaluer_clause(clause, list_var))

    if None in lst_result:
        return None
    if False in lst_result:
        return False
    else:
        return True
...
def determine_valuations(list_var):
    ...
    Renvoie : La liste de toutes les valuations (sans doublon) envisageables 
    pour les variables de list_var
    '''
    valuations = [list_var]
    for i in range(len(list_var)):
        if list_var[i] == None:
            ...
            for valuation in valuations:
                ...
                new_valuation.append(true_valuation)
                new_valuation.append(false_valuation)
...
def resol_sat_force_brute(formule,list_var):
    ...
    '''
    for valuation in determine_valuations(list_var):
...
def enlever_litt_for(formule,litteral):
    ...
    litteral : un entier non nul traduisant la valeur logique prise par 
    une variable
...
def init_formule_simpl_for(formule_init,list_var):
    ...
    for i in range(len(list_var)):
        if list_var[i] == False:
            result = enlever_litt_for(result, -(i+1))
...
def retablir_for(formule_init,list_chgmts):
    '''Arguments : une formule initiale et une liste de changements à apporter sur un ensemble de variables 
    (chaque changement étant une liste [i,bool] avec i l'index qu'occupe la variable dans list_var et bool 
    la valeur logique qui doit lui être assignée) 
...
def retour(list_var,list_chgmts):
    ...
    if list_chgmts == []:
        ...
        for changement in reversed(list_chgmts):
            if not changement[1]:
...
def retour_simpl_for(formule_init,list_var,list_chgmts):
    ...
    return init_formule_simpl_for(formule_init, list_var), list_var, list_chgmts
...
def resol_parcours_arbre(formule_init,list_var,list_chgmts):
    ...
    if evaluer_cnf(formule_init, list_var):
        ...
        list_var_teste, list_chgmts_teste = copy.deepcopy(list_var), copy.deepcopy(list_chgmts)
        list_var_teste, list_chgmts_teste = progress(list_var_teste, list_chgmts_teste)
        if list_var == list_var_teste and list_chgmts == list_chgmts_teste:
            list_var, list_chgmts = retour(list_var, list_chgmts)
